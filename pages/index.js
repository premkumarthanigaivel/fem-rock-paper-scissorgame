...
export default function Home() {
  ...
  const getMatchStatus = useCallback(() => {
    if (selectedSymbol === houseSymbol) {
      ...
      (selectedSymbol === SYMBOLS.SCISSOR && houseSymbol === SYMBOLS.PAPER)
    ) {
      setMatchScore(prevScore => prevScore + 1)
      setMatchStat("YOU WON")
    } else {
      setMatchScore(prevScore => (prevScore < 1 ? prevScore : prevScore - 1))
  ...
  return (
    <>
      ...
      <PageContainer>
        ...
        {!selectedSymbol && (
          <Page.SymbolSelector setSelectedSymbol={setSelectedSymbol} />
        )}
        {selectedSymbol && (
          <Page.GamePlay
